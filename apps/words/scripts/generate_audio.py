...
# ]
# ///
"""Generate OGG audio files for every unique word in words.json using Piper TTS."""
import hashlib
import json
import sys
...
WORDS_JSON = REPO_ROOT / "static/apps/words/words.json"
AUDIO_DIR = REPO_ROOT / "static/apps/words/audio"
...
def main() -> int:
    ...
    weeks = json.loads(WORDS_JSON.read_text())
    words = sorted({e["word"] for w in weeks for e in w["words"]})
